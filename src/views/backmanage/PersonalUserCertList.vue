<template>
    <a-card :bordered="false">
      ...
      <span slot="userName" slot-scope="text">
        ...
        <personal-user-cert-modal ref="modal" @ok="handleOk"/>
...
import PersonalUserCertModal from './modules/PersonalUserCertModal.vue'
import { checkPermission } from '@/utils/permissions'
import { getDictArray } from '@/utils/dict'
import pick from 'lodash.pick'
import { queryNationality } from '@/api/backmanage/nationality'
import Descriptions from '@/components/Descriptions/Descriptions'
import { personalAuthentication } from '../../api/backmanage/personaluser'
...
export default {
  ...
  async created () {
    /* 获取实名认证等级 */
    getDictArray('auth_level').then(reset => {
      this.authlevels = reset
  ...
  methods: {
    ...
    handleSucc () {
      ...
      personalAuthentication(this.mdl).then(res => {
        if (res.status === 0) {
    ...
    onChangeStatus (record) {
      ...
      savePersonalUserInfo(pick(record, 'id', 'status')).then(res => {
        if (res.code === 0) {
