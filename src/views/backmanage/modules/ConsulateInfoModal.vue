<template>
  ...
  >
    <a-form :form="form">
      ...
      <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="教育处名称">
        <a-input placeholder="教育处名称" v-decorator="['educationName']"/>
      </a-form-item>
...
<script>
import { saveConsulateInfo } from '@/api/backmanage/consulateuser'
import pick from 'lodash.pick'
export default {
  name: 'ConsulateInfoModal',
  props: {
  },
  components: {
  },
  ...
  methods: {
    ...
    edit (record) {
      ...
      this.visible = true
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.mdl, 'id','userName','phone','email','password','name','educationName','cardType','cardNo','avatarUrl'))
      })
    },
    handleSubmit (e) {
      e.preventDefault()
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          saveConsulateInfo(values).then(res => {
            if (res.code === 0) {
