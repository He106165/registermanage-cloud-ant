<template>
  ...
  >
    <a-form :form="form">
      ...
      <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="网格名称">
        <a-input placeholder="网格名称" v-decorator="['gridName']"/>
      </a-form-item>
      ...
      <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="证件类型">
        <a-input placeholder="证件号" v-decorator="['cardType']"/>
      </a-form-item>
      <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="头像地址">
        <a-input placeholder="头像地址" v-decorator="['avatarUrl']"/>
      </a-form-item>
...
<script>
import { saveGridUserInfo } from '@/api/backmanage/gridUser'
import pick from 'lodash.pick'
export default {
  name: 'GridUserInfoModal',
  props: {
  },
  components: {
  },
  ...
  methods: {
    ...
    edit (record) {
      ...
      this.visible = true
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.mdl, 'id','userName','phone','email','password','name','cardType','cardNo','avatarUrl','status','remark','createTime','createBy','updateTime','updateBy','version','delFlag','gridName'))
      })
    },
    handleSubmit (e) {
      e.preventDefault()
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          saveGridUserInfo(values).then(res => {
            if (res.code === 0) {
