...
<script>
import { saveInsideRole } from '@/api/backmanage/insiderole'
...
export default {
  ...
  methods: {
    ...
    edit (record) {
      ...
      this.visible = true
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.mdl, 'roleId','roleName','roleKey','sort','roleStatus','remark','createTime','createBy','updateTime','updateBy','version','delFlag'))
      })
    },
    handleSubmit (e) {
      e.preventDefault()
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          saveInsideRole(values).then(res => {
            if (res.code === 0) {
